...
async function api(action,data={}){if(!API_URL)throw new Error('Falta API_URL');const r=await fetch(API_URL,{method:'POST',body:JSON.stringify({action,...data})});const j=await r.json();if(!j.ok)throw new Error(j.error||'Operación no válida');return j}
...
function fillSelect(id,all=false){const s=document.getElementById(id);s.innerHTML=all?'<option value="">Todas las empresas</option>':'';empresasCache.forEach(e=>s.innerHTML+=`<option>${e.Empresa}</option>`)}
function initChecks(){document.getElementById('acts').innerHTML=ACTIVIDADES.map(a=>`<div class="check"><span>${a}</span><select data-act="${a}" required><option value="">Seleccione</option><option>SI</option><option>NO</option></select></div>`).join('');document.getElementById('sints').innerHTML=SINTOMAS.map(a=>`<div class="check"><span>${a}</span><select data-sint="${a}" required><option value="">Seleccione</option><option>SI</option><option>NO</option></select></div>`).join('')}
async function buscarTrabajador(){try{const d=dniBuscar.value.trim();if(!dniOk(d))return toast('DNI debe tener 8 dígitos');const r=await api('buscarTrabajador',{dni:d});if(!r.trabajador){datosTrab.classList.remove('hidden');datosTrab.innerHTML='<b>DNI no se encuentra activo en la master.</b>';formReporte.classList.add('hidden');return}trabajadorActual=r.trabajador;datosTrab.classList.remove('hidden');datosTrab.innerHTML=`<b>${r.trabajador.Nombres}</b><br>${r.trabajador.Cargo} · ${r.trabajador.Empresa} · ${r.trabajador.Sede}`;formReporte.classList.remove('hidden')}catch(e){toast(e.message)}}
formReporte.onsubmit=async ev=>{ev.preventDefault();try{if(!trabajadorActual)return toast('Busque primero al trabajador');let Actividades={},Sintomas={},missing=false;document.querySelectorAll('[data-act]').forEach(x=>{if(!x.value)missing=true;Actividades[x.dataset.act]=x.value});document.querySelectorAll('[data-sint]').forEach(x=>{if(!x.value)missing=true;Sintomas[x.dataset.sint]=x.value});if(missing||!obs.value.trim())return toast('Todos los campos son obligatorios');const con=Object.values(Sintomas).some(v=>v==='SI');const reporte={...trabajadorActual,DNI:String(trabajadorActual.DNI),Actividades,Sintomas,Observacion:obs.value.trim(),Condicion:con?'CON SÍNTOMAS':'SIN SÍNTOMAS'};await api('guardarReporte',{reporte});toast('Reporte registrado y PDF enviado al correo');formReporte.reset();formReporte.classList.add('hidden');datosTrab.classList.add('hidden')}catch(e){toast(e.message)}}
async function initEmpresa(){await loadEmpresas();fillSelect('temp');fillSelect('fempGestion',true);listarTrabajadores()}
async function guardarTrabajador(){try{const t={DNI:tdni.value.trim(),Nombres:tnom.value.trim(),Cargo:tcargo.value.trim(),Empresa:temp.value,Sede:tsede.value.trim(),Rubro:trubro.value.trim(),Correo:tcorreo.value.trim(),Estado:test.value};if(!dniOk(t.DNI))return toast('DNI debe tener 8 dígitos');if(Object.values(t).some(v=>!v))return toast('Todos los campos son obligatorios');await api('guardarTrabajador',{trabajador:t});toast('Trabajador guardado');listarTrabajadores()}catch(e){toast(e.message)}}
async function listarTrabajadores(){try{const r=await api('listarTrabajadores',{empresa:fempGestion.value});const arr=r.trabajadores||[];tablaTrab.innerHTML='<table class="table"><tr><th>DNI</th><th>Nombres</th><th>Cargo</th><th>Empresa</th><th>Estado</th><th>Acción</th></tr>'+arr.map(x=>`<tr><td>${x.DNI}</td><td>${x.Nombres}</td><td>${x.Cargo}</td><td>${x.Empresa}</td><td>${x.Estado}</td><td><button onclick='editTrab(${JSON.stringify(x)})'>Editar</button> <button class="danger" onclick="eliminarTrabajador('${x.DNI}')">Eliminar</button></td></tr>`).join('')+'</table>'}catch(e){toast(e.message)}}
function editTrab(x){tdni.value=x.DNI;tnom.value=x.Nombres;tcargo.value=x.Cargo;temp.value=x.Empresa;tsede.value=x.Sede;trubro.value=x.Rubro;tcorreo.value=x.Correo;test.value=x.Estado;scrollTo(0,0)}
async function eliminarTrabajador(dni){if(confirm('¿Eliminar trabajador?')){await api('eliminarTrabajador',{dni});toast('Eliminado');listarTrabajadores()}}
async function initEmpresas(){await loadEmpresas();tablaEmp.innerHTML='<table class="table"><tr><th>Empresa</th><th>Correo doctor</th></tr>'+empresasCache.map(e=>`<tr><td>${e.Empresa}</td><td>${e.CorreoDoctor}</td></tr>`).join('')+'</table>'}
